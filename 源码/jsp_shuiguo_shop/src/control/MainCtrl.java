...
public class MainCtrl extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("fbhd"))
		{
			...
			String filename = request.getParameter("filename");
			 ...
			 String sql = "insert into mixinfo values('"+title+"','"+filename+"','"+content+"','"+date+"') ";
				dao.commOper(sql);
			    request.setAttribute("suc", "");
				go("/admin/fbhd.jsp", request, response);
		}
		...
		if(ac.equals("uphd"))
		{
			...
			String filename = request.getParameter("filename");
             String content = request.getParameter("content");
			 String sql = "update mixinfo set title='"+title+"',filename='"+filename+"',content='"+content+"' where id= "+id;
				dao.commOper(sql);
			    request.setAttribute("suc", "");
				go("/admin/uphd.jsp?id="+id, request, response);
		}
		...
		if(ac.equals("pinj"))
		{
	        ...
	        response.setCharacterEncoding("utf-8");
             ...
             String proid = request.getParameter("proid");
			 String sql = "insert into pinj values(null,'"+user.get("uname")+"',"+pj+",'"+proid+"') ";
				dao.commOper(sql);
				go("/xiang.jsp?id="+proid, request, response);
		}
		...
		if(ac.equals("addgl"))
		{
			String uname = Info.getGB2312Str(request.getParameter("uname") );
		     String upass = Info.getUTFStr(request.getParameter("upass"));
		     String tname = Info.getUTFStr(request.getParameter("tname"));
		     String sex = Info.getUTFStr(request.getParameter("sex"));
		     String age = Info.getUTFStr(request.getParameter("age"));
		     String tel = Info.getUTFStr(request.getParameter("tel"));
		     String addr = Info.getUTFStr(request.getParameter("addr"));
		     String email = Info.getUTFStr(request.getParameter("email"));
		     String qq = Info.getUTFStr(request.getParameter("qq"));
				...
				go("/admin/glgl.jsp?id=", request, response);
		}
		...
		if(ac.equals("uploaddoc4"))
		{
			...
			go("/js/uploaddoc4.jsp?docname="+filename, request, response);
			} catch (Exception e1) {
				e1.printStackTrace();
			    }
		}
		
		if(ac.equals("uploaddoc5"))
		{
			try {
				String filename="";
			request.setCharacterEncoding("utf-8");
			...
			go("/js/uploaddoc5.jsp?docname="+filename, request, response);
			} catch (Exception e1) {
				e1.printStackTrace();
			    }
		}
